...
//
//  Created by Prianka Kariat on 25/08/21.
//
...
+ (uint8_t * _Nullable)convertBGRAtoRGBforPixelBufferBaseAddress
...
:(CVPixelBufferRef)pixelBuffer error:(NSError **)error {
  ...
  uint8_t *destPixelBufferAddress = [TFLCommonUtils mallocWithSize:height * destinationBytesPerRow error:error];
  ...
  convertError = vImageConvert_BGRA8888toRGB888(&srcBuffer,
                                                &destBuffer,
                                                kvImageNoFlags);
...
+ (UInt8 * _Nullable)pixelDataFromCGImage:(CGImageRef)cgImage error:(NSError **)error {
  ...
  if (context) {
    ...
    buffer_to_return = [UIImage populateRGBBufferFromSourceRGBABuffer:CGBitmapContextGetData(context)
                                               width:width
                                              height:height];
...
+ (UInt8 * _Nullable) populateRGBBufferFromSourceRGBABuffer:(UInt8 *)buffer
                                         width:(size_t)width
...
- (TfLiteFrameBuffer *)frameBufferFromCIImage:(CIImage *) ciImage
  ...
  if (ciImage.pixelBuffer) {
    buffer =
        [TFLCVPixelBufferUtils
            convertBGRAtoRGBforPixelBufferBaseAddress:ciImage.pixelBuffer error:error];
  ...
  switch (gmlImage.imageSourceType) {
    ...
    case GMLImageSourceTypeSampleBuffer:{
      ...
      cFrameBuffer = [GMLImageUtils bufferFromCVPixelBuffer
                      :sampleImagePixelBuffer error:error];
      break;
    }
...
+ (TfLiteFrameBuffer *)frameBufferFromUIImage:(UIImage *)image
  ...
  TfLiteFrameBuffer *cFrameBuffer = [image frameBufferWithError:error];
  return  cFrameBuffer;
}
...
+ (TfLiteFrameBuffer *)bufferFromCVPixelBuffer:(CVPixelBufferRef)pixelBuffer
  ...
  switch (pixelBufferFormat) {
    ...
    case kCVPixelFormatType_32BGRA:{
      ...
      buffer =
        [TFLCVPixelBufferUtils
         convertBGRAtoRGBforPixelBufferBaseAddress:pixelBuffer error:error];
      break;
    }
