...
using ::tflite::support::utils::kIOException;
...
jlong CreateAudioClassifierFromOptions(JNIEnv* env,
  ...
  if (audio_classifier_or.ok()) {
    ...
    auto error_code = audio_classifier_or.status().code();
    // These status are returned by ExternalFileHandler::MapExternalFile.
    if (error_code == absl::StatusCode::kNotFound ||
        error_code == absl::StatusCode::kPermissionDenied) {
      ThrowException(
          env, kIOException,
          "Error occurred when loading model file for AudioClassifier: %s",
...
// Creates an AudioClassifier instance from the model file path.
extern "C" JNIEXPORT jlong JNICALL
Java_org_tensorflow_lite_task_audio_classifier_AudioClassifier_initJniWithModelPathAndOptions(
    JNIEnv* env, jclass thiz, jstring model_path, jobject java_options) {
  AudioClassifierOptions proto_options =
      ConvertToProtoOptions(env, java_options);
  proto_options.mutable_base_options()->mutable_model_file()->set_file_name(
      JStringToString(env, model_path));
  return CreateAudioClassifierFromOptions(env, proto_options);
}

extern "C" JNIEXPORT jlong JNICALL
Java_org_tensorflow_lite_task_audio_classifier_AudioClassifier_initJniWithByteBufferAndOptions(
