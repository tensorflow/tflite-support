...
Java_org_tensorflow_lite_task_audio_classifier_AudioClassifier_classifyNative(
  ...
  const int sizeInBytes = env->GetArrayLength(java_array);
  const int sizeInFloat = sizeInBytes / sizeof(float);
  const StatusOr<std::unique_ptr<AudioBuffer>> audio_buffer_or =
      AudioBuffer::Create(reinterpret_cast<float*>(native_array), sizeInFloat,
                          format);
