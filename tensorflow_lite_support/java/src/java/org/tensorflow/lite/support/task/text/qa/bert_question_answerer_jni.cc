...
Java_org_tensorflow_lite_support_task_text_qa_BertQuestionAnswerer_initJniWithBertByteBuffers(
...
extern "C" JNIEXPORT jlong JNICALL
Java_org_tensorflow_lite_support_task_text_qa_BertQuestionAnswerer_initJniWithAlbertByteBuffers(
    JNIEnv* env, jclass thiz, jobjectArray model_buffers) {
  auto model =
      GetMappedFileBuffer(env, env->GetObjectArrayElement(model_buffers, 0));
  auto sp_model =
...
Java_org_tensorflow_lite_support_task_text_qa_BertQuestionAnswerer_answerNative(
  ...
  auto results = question_answerer->Answer(JStringToString(env, context),
                                           JStringToString(env, question));
  jclass qa_answer_class =
      env->FindClass("org/tensorflow/lite/support/task/text/qa/QaAnswer");
