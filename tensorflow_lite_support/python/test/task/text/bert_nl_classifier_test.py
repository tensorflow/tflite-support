...
class BertNLClassifierTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_classify_succeeds_with_long_positive_not_out_of_bounds(self):
    # Creates classifier.
    classifier = _BertNLClassifier.create_from_file(self.model_path)
    # BertNLClassifier limits the input sequence to `max_seq_len`, so we
    # verify if the classifier works correctly if the input is longer than
    # `max_seq_len`.
    positive_review_str = "it's a charming and often affecting journey " \
                          "and this is a long"
    for index in range(_MAX_SEQ_LEN):
      positive_review_str += "long"

    positive_review_str += " movie review"
