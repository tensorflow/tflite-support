...
class ModelMetadataPopulator {
 public:
  ...
  // Writes the provided TFLite ModelMetadata into the TFLite FlatBuffer model.
  //
  ...
  void LoadMetadata(const tflite::ModelMetadataT& model_metadata_t);
 ...
 private:
  ...
  // Zips and appends associated files to the model buffer. Called internally
  // by `Populate()`.
